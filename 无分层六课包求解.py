...
def build_model(
    ...
    # x[student_id, section_id] = whether the student is assigned to the section.
    x = {}
    for student in students:
    ...
    for type_key, type_sections in sections_by_type.items():
        ...
        for section in type_sections:
            ...
            # Minimize deviation from the exact average within the same subject+type.
            # Using |size * count - total| avoids fractions.
    ...
    # y[section_id, package] = whether the section is scheduled in the package.
    y = {}
    for section in sections:
        vars_for_section = []
    ...
    # 30 sections across 6 packages => 5 sections per package.
    for package in PACKAGES:
    ...
    # Break package symmetry.
    model.Add(y[(sections[0].id, 0)] == 1)

    # z[student_id, section_id, package] = x and y
    z = {}
    for student in students:
